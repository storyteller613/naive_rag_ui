...
class EmbeddingModel:
    def __init__(self, model_type="openai"):
        ...
        if model_type == "nomic":
            ...
            # Default Embedding model is ONNXMiniLM_L6_V2
    ...
    def generate_completion(self, messages):
        try:
            response = self.client.chat.completions.create(
                ...
                messages=messages,
                temperature=0.0, #0.0 is the most deterministic
            )
...
def streamlit_app():
    ...
    llm_type = st.sidebar.radio(
        "Select LLM Model:",
        ["ollama3.1", "ollama3.2"],
        format_func=lambda x: "Ollama llama3.1" if x == "ollama3.1" else "Ollama llama3.2",
    )
    ...
    if query:
        with st.spinner("Processing your query..."):
            ...
            with st.expander("🔍 Technical Details", expanded=False):
                ...
                st.write(f"- LLM Model: {llm_type.capitalize()}")
                st.write(f"- Embedding Model: {embedding_type.capitalize()}")
